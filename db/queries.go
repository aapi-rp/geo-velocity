...
const GetSubsequent = `select IP_ADDRESS,LAT,LONG,RADIUS,LOGIN_TIME from GEO_VELOCITY_EVENTS WHERE LOGIN_TIME > ? AND USERNAME=? ORDER BY LOGIN_TIME ASC LIMIT 1`
const GetPrevious = `select IP_ADDRESS,LAT,LONG,RADIUS,LOGIN_TIME from GEO_VELOCITY_EVENTS WHERE LOGIN_TIME < ? AND USERNAME=? ORDER BY LOGIN_TIME DESC LIMIT 1`
